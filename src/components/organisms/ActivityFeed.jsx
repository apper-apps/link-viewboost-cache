...
const ActivityFeed = React.memo(({ activities = [] }) => {
...
<div className="p-4 sm:p-6">
        {activities.length === 0 ? (
          ...
          <div className="space-y-3 max-h-80 sm:max-h-96 overflow-y-auto overscroll-y-contain">
            {activities.slice(0, 20).map((activity, index) => (
              <motion.div
                ...
                className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-800/50 transition-colors touch-manipulation"
              >
                <div className={`p-2 rounded-lg bg-gray-800 flex-shrink-0 ${getActivityColor(activity.severity)}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1">
                    ...
                    <Badge variant={getBadgeVariant(activity.severity)} className="flex-shrink-0 w-fit">
                      {activity.severity}
                    </Badge>
                  </div>
